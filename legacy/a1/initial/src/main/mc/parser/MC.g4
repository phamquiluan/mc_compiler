...
// -------------- PARSER -----------------
program: (vardec|funcdec)+ EOF;

// declaration
vardec :vartype varid (CM varid)* SEMI;

vartype : INTTYPE|BOOLEAN|FLOATTYPE|STRING;
arrtype : vartype LSB INTLIT RSB;
arptype : vartype LSB RSB;
fultype : arptype|vartype|VOIDTYPE;
varid 	: ID (LSB INTLIT RSB)?;

funcdec : fultype ID LB plist? RB block_s;
